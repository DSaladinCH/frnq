	...
	>
		<div class="flex h-full flex-col">
			...
			{#each mainLinks as { key, icon, label }, i}
				<button
					type="button"
					class="h-12.5 w-12.5 hover:scale-115 my-1 flex flex-col items-center justify-center text-2xl transition-transform hover:cursor-pointer group"
					aria-label={label}
					title={label}
					onclick={() => navigateTo(key)}
				>
					<i class="{icon} {keyActive(key)} gradient-text leading-none group-hover:gradient-text-hover"></i>
			...
			{#each footerLinks as { key, icon, label }, i}
				<button
					type="button"
					class="h-12.5 w-12.5 flex flex-col items-center justify-center text-2xl group {i === 0
						? 'mt-auto'
					...
					onclick={() => navigateTo(key)}
				>
					<i class="{icon} {keyActive(key)} gradient-text group-hover:gradient-text-hover"></i>
					<span class="display md:hidden">{label}</span>
				</button>
